...
const DEFAULT_DISABLE_HTTP2 = true;
const DEFAULT_NAV_WAIT_UNTIL: NonNullable<Parameters<import('playwright').Page['goto']>[1]>['waitUntil'] =
  'commit';
const DEFAULT_POST_NAV_WAIT_MS = 750;
...
async function scrapeProductPage(
  ...
  url: string,
  timeoutMs: number,
  navWaitUntil: NonNullable<Parameters<import('playwright').Page['goto']>[1]>['waitUntil'],
  ...
  try {
    ...
    try {
      await page.goto(url, { waitUntil: navWaitUntil, timeout: timeoutMs });
      if (navWaitUntil !== 'domcontentloaded') {
        await page.waitForLoadState('domcontentloaded', { timeout: Math.min(timeoutMs, 15000) }).catch(() => {});
      }
    } catch (error) {
      console.warn(`Primary navigation failed for ${url}:`, error);
      const fallbackHtml = await fetchHtmlWithContext(context, url, timeoutMs);
      if (fallbackHtml) {
        const product = findProductData(fallbackHtml);
        return product ? parseProviderPrice(product) : null;
      }
      return null;
    }
...
async function fetchHtmlWithContext(context: BrowserContext, url: string, timeoutMs: number) {
  try {
    const response = await context.request.get(url, { timeout: timeoutMs });
    if (!response.ok()) {
      console.warn(`Request failed for ${url}: ${response.status()} ${response.statusText()}`);
      return null;
    }
    return await response.text();
  } catch (error) {
    console.warn(`Request fetch failed for ${url}:`, error);
    return null;
  }
}
...
function parseArgs() {
  ...
  let limit = 200;
  let force = false;
  let batchSize = Number(process.env.SCRAPER_BATCH_SIZE) || 3;
  let timeoutMs = Number(process.env.SCRAPER_TIMEOUT_MS) || DEFAULT_TIMEOUT_MS;
  let headful = false;
  let saveSession = false;
  let navWaitUntil = (process.env.SCRAPER_NAV_WAIT_UNTIL as typeof DEFAULT_NAV_WAIT_UNTIL) ?? DEFAULT_NAV_WAIT_UNTIL;
  let postNavWaitMs = Number(process.env.SCRAPER_POST_NAV_WAIT_MS) || DEFAULT_POST_NAV_WAIT_MS;
  let blockResources =
    process.env.SCRAPER_BLOCK_RESOURCES === undefined
      ? DEFAULT_BLOCK_RESOURCES
      : process.env.SCRAPER_BLOCK_RESOURCES.trim().toLowerCase() !== 'false';
  ...
  args.forEach((arg) => {
    if (arg.startsWith('--stores=')) {
      ...
      navWaitUntil = arg.split('=')[1] as typeof DEFAULT_NAV_WAIT_UNTIL;
    } else if (arg.startsWith('--post-wait=')) {
      postNavWaitMs = Number(arg.split('=')[1]);
...
async function main() {
  ...
  const { stores, limit, force, batchSize, timeoutMs, headful, saveSession, navWaitUntil, postNavWaitMs, blockResources } =
    parseArgs();
  const storageStatePath = process.env.SCRAPER_STORAGE_STATE || DEFAULT_STORAGE_STATE;
  const storageStateExists = fs.existsSync(storageStatePath);

  if (!storageStateExists) {
    console.log(`Storage state not found at ${storageStatePath}. Running without cookies.`);
  }
  ...
  const disableHttp2Raw = process.env.SCRAPER_DISABLE_HTTP2;
  const disableHttp2 =
    disableHttp2Raw === undefined
      ? DEFAULT_DISABLE_HTTP2
      : disableHttp2Raw.trim().toLowerCase() !== 'false';
  const launchArgs = disableHttp2 ? ['--disable-http2', '--disable-quic'] : [];
  ...
  if (saveSession || storageStateExists) {
